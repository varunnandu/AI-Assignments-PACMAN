...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        all_possible_positions = util.Counter()
        for p in self.legalPositions:
            actual_distance = util.manhattanDistance(p, pacmanPosition)
            if emissionModel[actual_distance] > 0:
                all_possible_positions[p] = emissionModel[actual_distance] * self.beliefs[p]

        "*** END YOUR CODE HERE ***"

        all_possible_positions.normalize()
        self.beliefs = all_possible_positions
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                all_possible_positions[newPos] += prob * self.beliefs[p]
        all_possible_positions.normalize()
        self.beliefs = all_possible_positions
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        lenLP = len(self.legalPositions)
        for par in range(0, numPar):
            self.list_of_particles.append(self.legalPositions[par % lenLP])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for n in range(0, self.numParticles):
                self.list_of_particles[n] = jp
        else:
            allPossible = util.Counter()
            beliefsDist = self.getBeliefDistribution()
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = allPossible[p] + (emissionModel[trueDistance] * beliefsDist[p])
            w = 0.0
            for loc, prob in allPossible.items():
                if prob > 0.0:
                    w = prob
            if w == 0.0:
                self.initializeUniformly(gameState)
                ...
                for n in range(0, self.numParticles):
                    self.list_of_particles[n] = util.sample(allPossible)
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.list_of_particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            self.list_of_particles[x] = util.sample(newPosDist)
            x += 1
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefsDist = util.Counter()
        for p in self.list_of_particles:
            beliefsDist[p] = beliefsDist[p] + 1
        beliefsDist.normalize()
        return beliefsDist
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        legal_position = self.legalPositions
        number_of_ghosts = self.numGhosts
        all_possible_positions = list(itertools.product(legal_position, repeat = number_of_ghosts))
        count = 0
        self.particles =  []
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        "*** YOUR CODE HERE ***"
        all_possible_positions = util.Counter()
        for ind, p in enumerate(self.particles):
            reading = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    reading *= emissionModels[i][distance]
            all_possible_positions[p] += reading

        if any(all_possible_positions.values()):
            all_possible_positions.normalize()
            list1 = []
            for __ in range(0, self.numParticles):
                list1.append(util.sample(all_possible_positions))
